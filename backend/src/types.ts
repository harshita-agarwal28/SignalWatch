...
export interface MarketDataProvider {
  start(): Promise<void>
  /**
   * Stop ticking and release the interval. Required so the provider can be
   * swapped at runtime without leaving the old engine running - without
   * this, toggling simulated/live would leave two engines ticking at once,
   * both writing to the same symbol rows and fighting each other.
   */
  stop(): void
